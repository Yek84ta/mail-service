        ...
        searchButton.addActionListener(e -> {
            ...
            if (code != null && !code.trim().isEmpty()) {
                try (Session session = HibernateUtil.getSessionFactory().openSession()) {
                    ...
                    if (mail.isPresent()) {
                        boolean isRecipient = mail.get().getRecipients().stream()
                                .anyMatch(r -> r.getId() == currentUser.getId());
                        boolean isSender = mail.get().getSender().getId() == currentUser.getId();
                        ...
                        JOptionPane.showMessageDialog(this,
                                "No email found with code: " + code,
                                "Not Found", JOptionPane.INFORMATION_MESSAGE);
                    }
                } catch (Exception ex) {
                    logger.log(Level.SEVERE, "Error searching mail", ex);
                    JOptionPane.showMessageDialog(this,
                            "Error searching for email: " + ex.getMessage(),
                            "Error", JOptionPane.ERROR_MESSAGE);
                }
            }
        });
